...
from neus_network import DummyEmbedder, Embedder, MLPNet, SDFNetwork, RenderingNetwork, SingleVarianceNetwork
...
# def render_single_image(rank, world_size, models, ray_sampler, chunk_size, iteration, args):
...
#             with autocast():
#                 # with torch.no_grad():
#                     ret = net(ray_o, ray_d, near, far, iteration, img_name=ray_sampler.img_path)

#             # print("----- Before Ret ---", ret)

#             for key in ret:
#                 if key not in ['s_val', 'weight_sum','weight_max']:
...
#             # Reshape zero-dimensional tensors to one-dimensional
#             ret_merge_chunk[m][key] = [t.unsqueeze(0) if t.ndim == 0 else t for t in ret_merge_chunk[m][key]]
...
#                 ret_merge_rank[m][key] = ret_merge_rank[m][key].reshape((ray_sampler.H, ray_sampler.W, -1)).squeeze()
        ...
        for m in range(models['cascade_level']):
            ...
            with autocast():
                ...
                if m == 0:
                    ...
                    """This line computes the depth at which each ray 
                    in ray_batch intersects a sphere that encompasses the scene."""
                    ...
                    # near = torch.squeeze(near)
                    # far = torch.squeeze(far)
...
def config_parser():
    ...
    parser.add_argument("--n_importance", type=int, default=64, help='number of additional samples used ofr importance sampling')
