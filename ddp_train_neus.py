...
from ddp_model_neus import NeusRenderer, NeuSNetWrapper
import time
...
def intersect_sphere(ray_o, ray_d):
    ...
    d1 = -torch.sum(ray_d * ray_o, dim=-1) / torch.sum(ray_d * ray_d, dim=-1)  #unit vector in the direction of ray_d
...
def near_far_from_sphere(ray_o, ray_d):
    a = torch.sum(ray_d**2, dim=-1, keepdim=True)
    b = 2.0 * torch.sum(ray_o * ray_d, dim=-1, keepdim=True)
    mid = 0.5 * (-b) / a
    near = mid - 1.0
    far = mid + 1.0
    return near, far
...
def render_single_image(rank, world_size, models, ray_sampler, chunk_size, iteration):
    ...
    for s in range(len(ray_batch_split['ray_d'])):
        ...
        for m in range(models['cascade_level']):
            ...
            if m == 0:
                near, far = near_far_from_sphere(ray_o, ray_d)
            ...
            with autocast():
                ...
                ret = net(ray_o, ray_d, near, far, iteration,
                                background_rgb = background_rgb, perturb_overwrite = -1)

            for key in ret:
                if key not in ['s_val', 'cdf_fine', 'weight_sum', 'weight_max', 'gradient_error', 'inside_sphere']:  ###########(IF _____ in ()) Only Color channel
                    if torch.is_tensor(ret[key]):
                        if key not in ret_merge_chunk[m]:
                            ret_merge_chunk[m][key] = [ret[key].detach().cpu(), ]
                        else:
                            ret_merge_chunk[m][key].append(ret[key].detach().cpu())
...
def log_view_to_tb(writer, global_step, log_data, gt_events, gt_rgb, mask, prefix=''):
    ...
    for m in range(len(log_data)):
        ...
        rgb_im = img_HWC2CHW(log_data[m]['color_fine'])
        writer.add_image(prefix + 'level_{}/color_fine_norm'.format(m), (rgb_im-rgb_im.min(2, True)[0].min(1, True)[0])/(0.001+rgb_im.max(2,True)[0].max(1,True)[0]-rgb_im.min(2,True)[0].min(1,True)[0]), global_step)
        rgb_im = torch.clamp(rgb_im, min=0., max=1.)  # just in case diffuse+specular>1
        writer.add_image(prefix + 'level_{}/color_fine'.format(m), rgb_im, global_step)
        ...
        weights = log_data[m]['weights']
        writer.add_histogram(f'{prefix}level_{m}/weights', weights, global_step)

        normal_im = img_HWC2CHW(log_data[m]['normals'])
        writer.add_image(prefix + 'level_{}/normals_norm'.format(m), (normal_im-normal_im.min(2, True)[0].min(1, True)[0])/(0.001+normal_im.max(2,True)[0].max(1,True)[0]-normal_im.min(2,True)[0].min(1,True)[0]), global_step)
        normal_im = torch.clamp(normal_im, min=0., max=1.)  # just in case diffuse+specular>1
        writer.add_image(prefix + 'level_{}/normals'.format(m), normal_im, global_step)
...
def create_nerf(rank, args, camera_mgr, load_camera_mgr=True):
    ...
    for m in range(models['cascade_level']):
        net = NeuSNetWrapper(args, optim_autoexpo=args.optim_autoexpo, img_names=img_names).to(rank)
    ...
    for global_step in range(start + 1, start + 1 + args.N_iters):
        ...
        for m in range(models['cascade_level']):
            ...
            with autocast():
                ...
                N_samples = models['cascade_samples'][m]
                if m == 0:
                    near, far = near_far_from_sphere(ray_batch['ray_o'], ray_batch['ray_d'])

                # background_rgb = torch.zeros([1, 3]).cuda()
                # background_rgb = None
                background_rgb = torch.full([1, 3], 0.5).cuda()
                
                if not args.is_rgb_only:
                    prev_ret = net(ray_batch['prev_ray_o'], ray_batch['prev_ray_d'], near, far, global_step,
                                   background_rgb = background_rgb, perturb_overwrite = -1, img_name=ray_batch['img_name'])

                ret = net(ray_batch['ray_o'], ray_batch['ray_d'],  near, far, global_step,
                          background_rgb = background_rgb, perturb_overwrite = -1, img_name=ray_batch['img_name'])

                # color_fine = ret['color_fine']
                s_val = ret['s_val']
                cdf_fine = ret['cdf_fine']
                gradient_error = ret['gradient_error']
                weight_max = ret['weight_max']
                weight_sum = ret['weight_sum']
                normals = ret['normals']
                ...
                if not args.is_rgb_only:
                    ...
                    diff = torch.log(ret['color_fine']**2.2+eps)-torch.log(prev_ret['color_fine']**2.2+eps)

                else:
                    diff = ret['color_fine']*0
                ...
                if args.is_rgb_only:
                    ...
                    rgb_loss = img2mse(ret['color_fine'], rgb_gt, mask=mask_gt)
                    event_loss = rgb_loss
                ...
                if args.use_reg:
                    ...
                    loss = loss + eikonal_loss * args.eikonal_reg_weight

                scalars_to_log['level_{}/loss'.format(m)] = loss.item()
                scalars_to_log['level_{}/psnr'.format(m)] = mse2psnr(event_loss.item())
                scalars_to_log['level_{}/random_loss'.format(m)] = event_random_loss.item()
                
                # scalars_to_log['level_{}/weight_max'.format(m)] = weight_max.item()
                # scalars_to_log['level_{}/weight_sum'.format(m)] = weight_sum.item()

                if args.use_reg:
                    scalars_to_log['level_{}/eikonal'.format(m)] = eikonal_loss.item()
...
def config_parser():
    ...
    parser.add_argument("--use_reg", type=str2bool, default=True)
    ...
    parser.add_argument("--eikonal_reg_weight", type=float, default=0.1)
    ...
    parser.add_argument("--color_d_feature", type=int, default=256, help='dimensionality of the feature vector in rendering network')
    parser.add_argument("--color_mode", type=str, default='idr', help='Mode of operation, ex.(idr, no_view_dir, no_normal)')
    # parser.add_argument("--color_input_dim", type=int, default=9, help='input dimension for rendering network')
    parser.add_argument("--color_output_dim", type=int, default=3, help='output dimension for rendering network')
    parser.add_argument("--color_hidden_dim", type=int, default=256, help='dimension of hidden layers in rendering network')
    parser.add_argument("--color_n_layers", type=int, default=4, help='number of layers in the rendering network')
    parser.add_argument("--color_weight_norm", type=str2bool, default=True, help='if weight normalization should be used')
    parser.add_argument("--color_squeeze_out", type=str2bool, default=True, help='if the sigmoid activation should be applied to the output')

    parser.add_argument("--sdf_bias", type=float, default=0.5, help='bias term in sdf network')
    parser.add_argument("--sdf_scale", type=float, default=1.0, help='scaling term in sdf network')
    parser.add_argument("--sdf_d_out", type=int, default=257, help='dimensionality of the output layer in sdf network')
    parser.add_argument("--sdf_geometric_init", type=str2bool, default=True, help='whether to use geometric initialization for the networks weights')
    parser.add_argument("--sdf_weight_norm", type=str2bool, default=True, help='whether or not to apply weight normalization to the layers of the network')
    parser.add_argument("--sdf_inside_outside", type=str2bool, default=False, help='inside_outside flag toggles the sign of the weight initialization for the final layer of the network when geometric_init is active.')

    parser.add_argument("--dev_init_val", type=float, default=0.3, help='initial value for the deviation network')

    parser.add_argument("--n_samples", type=int, default=64, help='number of samples used for NeuS rendering')
    parser.add_argument("--n_importance", type=int, default=64, help='number of additional samples used for importance sampling')
    parser.add_argument("--n_outside", type=int, default=32, help='number of samples taken outside the SDF region')
    parser.add_argument("--up_sample_steps", type=int, default=4, help='number of steps used in the up-sampling process')
    parser.add_argument("--perturb", type=float, default=1.0, help='amount of perturbation or noise added to the depth samples')
...
def train():
    ...
    # torch.multiprocessing.spawn(ddp_train_neus,
    #                             args=(args,),
    #                             nprocs=args.world_size,
    #                             join=True)
    ddp_train_neus(0, args)
...
"""    torch.cat(embedder_position(pts, iteration), dim=-1)
