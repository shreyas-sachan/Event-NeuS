...
class RaySamplerSingleEventStream(object):
    ...
    def get_all(self):
        ...
        return ret
    # 
    # def random_sample(self, N_rand, neg_ratio=0):
    #     '''
    #     :param N_rand: number of rays to be cast
    #     :param neg_ratio: ratio of background (negative) samples to select
    #     :return:
    #     '''
    #     # Flatten the event frame and mask for easy selection
    #     if self.mask is not None:
    #         mask_flat = self.mask.flatten()  # Flatten the mask
    #     else:
    #         logger.warning('No mask found, sampling from all events')
    #         mask_flat = np.ones((self.H * self.W,))

    #     # Get indices of foreground and background based on mask
    #     fg_indices = np.where(mask_flat > 0.5)[0]  # Foreground where mask > 0.5
    #     bg_indices = np.where(mask_flat <= 0.5)[0]  # Background where mask <= 0.5

    #     # Sample foreground and background events
    #     fg_count = int(N_rand * (1 - neg_ratio))  # How many to sample from foreground
    #     bg_count = N_rand - fg_count  # The rest are background

    #     # Sample from foreground
    #     if len(fg_indices) > 0:
    #         fg_sample = np.random.choice(fg_indices, size=min(fg_count, len(fg_indices)), replace=False)
    #     else:
    #         logger.warning('No foreground events found')
    #         fg_sample = np.array([])

    #     # Sample from background
    #     if len(bg_indices) > 0:
    #         bg_sample = np.random.choice(bg_indices, size=min(bg_count, len(bg_indices)), replace=False)
    #     else:
    #         logger.warning('No background events found')
    #         bg_sample = np.array([])

    #     # Combine foreground and background indices
    #     select_inds = np.concatenate([fg_sample, bg_sample])

    #     # Now that we have selected indices, retrieve corresponding rays and events
    ...
    #     # Retrieve event data, RGB, and masks
    #     events = self.event_frame[select_inds, :] if self.events is not None else None
    #     rgb = self.rgb[select_inds, :] if self.rgb is not None else None
    #     prev_rgb = self.prev_rgb[select_inds, :] if self.prev_rgb is not None else None
    #     mask = self.mask[select_inds] if self.mask is not None else None
        
    #     min_depth = 1e-4 * torch.ones_like(rays_d[..., 0])

    #     color_mask = self.color_mask[select_inds, :]

    #     # Organize output into a dictionary and return
    #     ret = OrderedDict([
    #         ('ray_o', rays_o),
    #         ('ray_d', rays_d),
    #         ('depth', depth),
    ...
    #         ('mask', mask),  # Include the mask in the output
    #     ])
